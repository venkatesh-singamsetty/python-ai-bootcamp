...
st.title("🛡️ Social Ad Buy Predictor (Random Forest)")

# Load model and scaler
path = os.path.dirname(__file__)
model_path = os.path.join(path, "random_forest_model.joblib")
scaler_path = os.path.join(path, "scaler_rf.joblib")

if not os.path.exists(model_path):
    st.error("Model artifacts not found. Please run 'random_forest_classification_model.py' first.")
    st.stop()
