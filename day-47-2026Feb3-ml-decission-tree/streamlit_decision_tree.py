...
st.title("🌳 Social Ad Buy Predictor (Decision Tree)")

# Load model and scaler
path = os.path.dirname(__file__)
model_path = os.path.join(path, "decision_tree_model.joblib")
scaler_path = os.path.join(path, "scaler_dt.joblib")

if not os.path.exists(model_path):
    st.error("Model artifacts not found. Please run 'decision_tree_classification_model.py' first.")
    st.stop()
