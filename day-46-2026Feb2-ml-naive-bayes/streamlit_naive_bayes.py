...
st.title("⚖️ Social Ad Buy Predictor (Naive Bayes)")

# Load model and scaler
path = os.path.dirname(__file__)
model_path = os.path.join(path, "naive_bayes_model.joblib")
scaler_path = os.path.join(path, "scaler_std.joblib")

if not os.path.exists(model_path):
    st.error("Model artifacts not found. Please run 'naive_bayes_classification_model.py' first.")
    st.stop()
